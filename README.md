...
ヘッダー右側の設定ボタンから各種設定を確認し、**Start**ボタンをクリックすると音声認識が開始します。  
この際、設定ボタンの隣に存在する翻訳のアイコンが緑色になっていると、自動的に翻訳されます。

`v0.1.8`からは、左側のエリアにテキスト入力が可能になりました。  
文章を入力しEnterキーを押下すると、音声認識した際と同様の動作をします。
